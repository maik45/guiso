...
// $consulta = "SELECT idArticulo,nombre,linea,unidad,unidadA,factor,minimo,maximo,costo FROM articulo ORDER BY nombre";

// $resultado = mysqli_query($conexion,$consulta);

// $json = [];
// while( $json[] = mysqli_fetch_array($resultado) );
// array_pop($json);
...
// $sqlCount = "SELECT COUNT(folio) AS total FROM articulo AS art LEFT JOIN linea as li ON li.idLinea = art.linea WHERE art.activo = 1  ";

// $result = $db->query( $sqlCount );
// $totalData = $totalFiltered = $result[0]->total;
...
//   "data" => $result, // Objeto que contiene la tabla a ser mostrada
// );
