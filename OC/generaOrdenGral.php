...
function headerExcel( $data, &$indexRow ){
  ...
  $sheet->setCellValue("B{$indexRow}", implode(', ', array_keys( $data->unidades) ) );
...
foreach ($articulosVendidos as $key => $proveedor) {
  ...
  foreach ($proveedor as $unidadesProveedor) {    
    ...
    foreach ($unidadesProveedor as $articulo) {
      ...
      $sql = "SELECT cantidad FROM excedente WHERE articulo = '{$articulo->idArticulo}' AND unidad = '{$articulo->unidad}' LIMIT 1";
      $exd = $db->query($sql);
      ...
      if( array_key_exists($articulo->idArticulo, $itemsToDraw) ){

        if( array_key_exists( $articulo->unidadName, $itemsToDraw[$articulo->idArticulo] ) ){
          //si existe esa unidad en el array asocc acumulamos valores
          $itemsToDraw[$articulo->idArticulo][$articulo->unidadName] += $cantidad;
        }
        else{
        ...
        $art['precio'] = $articulo->costo;

        $art[$articulo->unidadName] = $cantidad;
        
        $itemsToDraw[$articulo->idArticulo] = $art;
...
header("Content-disposition: attachment; filename=OC General $start - $end.xlsx");
