...
// $sql = "SELECT me.idMenu, me.unidad, (SELECT unidad FROM unidad WHERE idUnidad = me.unidad) AS unidadName, mr.receta, mr.fecha, me.grupo, mr.personas, mr.pos, mr.precio FROM menu as me inner join menurec as mr on me.idMenu = mr.idMenu WHERE ( date(mr.fecha) between '{$start}' AND '{$end}' ) AND me.activo = 1";
// $r = $db->query($sql);

$sql = "SELECT * FROM bomoc WHERE ( fechaI BETWEEN '{$start}' AND '{$end}' ) AND ( fechaF BETWEEN '{$start}' AND '{$end}') ORDER BY OC, articulo, fechaI";

$resultBom = $db->query($sql);
...
$unidades = [];
// echo "<pre>";
while( $item = $resultBom->fetch_object() ){
  ...
  if( array_key_exists( $item->proveedor, $articulosVendidos ) ){

    if( array_key_exists($item->unidad, $articulosVendidos[$item->proveedor]) ){//si existe la unidad en el proveedor

      if( array_key_exists( $item->articulo, $articulosVendidos[$item->proveedor][$item->unidad] ) ){
        //si existe acumulalo
        $articulosVendidos[$item->proveedor][$item->unidad][$item->articulo]->cantidad += $item->cantidad;
      }
      else{
        //si no existe el articulo en la unidad agregalo
        $r = $db->query("SELECT unidad FROM unidad WHERE idUnidad = '{$item->unidad}'");
        $item->unidadName = $r->fetch_object()->unidad;
        
        $r = $db->query("SELECT nombre, unidad FROM articulo WHERE idArticulo = '{$item->articulo}' LIMIT 1");
        $r = $r->fetch_object();  
        $item->nombre = $r->nombre;
        $item->artUnidad = $r->unidad;
        
        $articulosVendidos[$item->proveedor][$item->unidad][$item->articulo] = $item;
      }
    }
    else{
  ...
  //recuperamos la unidades existentes
  if( ! array_key_exists($item->unidad, $unidades) ){
    // echo var_dump($item);
    $unidades[$item->unidad] = ['id'=> $item->unidad, 'name'=> $item->unidadName];
  }
...
//         if( array_key_exists( $articulo->idArticulo, $articulosVendidos[$proveedorId][$articulo->unidad] ) ){
//           //si existe acumulalo
//           $articulosVendidos[$proveedorId][$articulo->unidad][$articulo->idArticulo]->cantidadCalc += $articulo->cantidadCalc;
//         }
//         else{
//           //si no existe el articulo en la unidad agregalo
//           $articulosVendidos[$proveedorId][$articulo->unidad][$articulo->idArticulo] = $articulo;
//         }
//       }
//       else{
//         //si no existe la unidad en el proveedor, lo agregamos
...
//     $unidades[$menu->unidad] = ['id'=> $menu->unidad, 'name'=> $menu->unidadName];
...
function headerExcel( $data, &$indexRow ){
  ...
  $sheet->setCellValue("B{$indexRow}", 'Todos');
...
foreach ($articulosVendidos as $key => $proveedor) {
  ...
  foreach ($proveedor as $unidadesProveedor) {    
    ...
    foreach ($unidadesProveedor as $articulo) {
      ...
      $cantidad = $articulo->cantidad - $exd;
      $costoT = $articulo->costoU * $cantidad;

      if( $costoT < 0 ) $costoT = 0;//si el costoU es negativo lo igualamos a 0
...
header("Content-disposition: attachment; filename=OC General Pres. $start - $end.xlsx");
