...
while( $itemUnidad = $unidadResult->fetch_object() ){
  ...
  while( $itemMenu = $menuResult->fetch_object() ){
    ...
    $articulosResult = $db->query( "SELECT re.porciones, reart.cantidad, art.idArticulo, art.nombre, art.costo, art.unidad, art.unidadA as presentacion, art.linea AS lineaId, pro.idProveedor, pro.nombre as proveedor, (SELECT descripcion FROM linea WHERE idLinea = art.linea LIMIT 1) as linea from receta as re join recetaart as reart on re.idReceta=reart.receta join articulo as art on art.idArticulo=reart.articulo join precioprov as preprov on ( art.idArticulo = preprov.articulo AND art.costo = preprov.precio) join proveedor as pro on pro.idProveedor = preprov.proveedor where reart.receta = '{$idReceta}'" );
    ...
    while( $itemArticulo = $articulosResult->fetch_object() ){
      ...
      $proveedor = $itemArticulo->proveedor;
      $idProveedor = $itemArticulo->idProveedor;
...
$sql = "SELECT un.idUnidad, mr.personas, mr.precio, re.idReceta, re.porciones, reart.cantidad, art.idArticulo, art.nombre, art.costo, art.unidad, art.unidadA as presentacion, art.linea AS lineaId, pro.idProveedor, pro.nombre as proveedor, (SELECT descripcion FROM linea WHERE idLinea = art.linea LIMIT 1) as linea
  FROM 
  unidad as un 
  inner join menu as me on un.idUnidad=me.unidad
  inner join menurec as mr on mr.idMenu=me.idMenu
  inner join receta as re on re.nombre=mr.receta
  inner join recetaart as reart on reart.receta=re.idReceta
  inner join articulo as art on art.idArticulo=reart.articulo
  inner join precioprov as preprov on ( preprov.articulo=art.idArticulo AND art.costo = preprov.precio )
  inner join proveedor as pro on pro.idProveedor=preprov.proveedor
